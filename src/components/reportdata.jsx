...
const ReportsData = ({ reports, handleDownload }) => {
  const handleDownloadAndPrint = (report) => {
    handleDownload(report); // Initiate file download

    // Wait for a short delay to ensure download completes (adjust delay as needed)
    setTimeout(() => {
      ...
      <tbody>
        {reports.map((report, index) => (
          <tr key={index} className="flex flex-row lg:space-x-[108px] md:space-x-[58px] space-x-2 my-8 mx-5">
            ...
            <td>{report.name}</td>
            <td>
              <button onClick={() => handleDownloadAndPrint(report)}>
